  ...
  return (
    <nav className="flex-between w-full mb-16 pt-3">
      <Link href="/" className="flex gap-2 flex-center">
        <Image src="/assets/images/logo.svg" width={50} height={50} alt="Promptopia Logo" className="object-contain" />
        ...
        {session?.user ? (
          <div className="flex gap-4 md:gap-5">
            ...
            <Link href="/profile" className="nav_link">
              <Image src={session?.user.image} width={37} height={37} alt="User Icon" className='rounded-full' />
            </Link>
          </div>
        ) : (
          <>
      ...
      <div className="sm:hidden flex relative">
        {
          session?.user ? (
            <div className="flex">
              <Image src="/assets/icons/menu.svg" width={37} height={37} alt="User Icon" className='rounded-full' onClick={() => setToggleDropdown((prevState) => !prevState)} />
              {toggleDropdown && (
                <div className="dropdown rounded-md shadow-md">
